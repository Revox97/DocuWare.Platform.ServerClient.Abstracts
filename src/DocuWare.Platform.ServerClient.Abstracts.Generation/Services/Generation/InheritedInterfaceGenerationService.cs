...
{
    internal class InheritedInterfaceGenerationService(string? namespaceExtension = null) : IGenerationService
    {
        ...
        public void Generate(Type type)
        {
            ...
            // TODO avoid duplicate code generation
        ...
        {
            PropertyInfo[] baseProperties = [.. type.BaseType!.GetProperties(BindingFlags.Instance | BindingFlags.Public).Where(p => !p.CustomAttributes.Any(a => a.AttributeType == typeof(XmlIgnoreAttribute)))];
            PropertyInfo[] properties = [.. type.GetProperties(BindingFlags.Instance | BindingFlags.Public).Where(p => !p.CustomAttributes.Any(a => a.AttributeType == typeof(XmlIgnoreAttribute)) && !baseProperties.Any(b => b.Name.Equals(p.Name)))];
        ...
        {
            MethodInfo[] baseMethods = [.. type.BaseType!.GetMethods(BindingFlags.Instance | BindingFlags.Public).Where(m => !m.IsSpecialName)];
            MethodInfo[] methods = [.. type.GetMethods(BindingFlags.Instance | BindingFlags.Public).Where(m => !m.IsSpecialName && !baseMethods.Any(b => b.Name.Equals(m.Name) && b.GetParameters().SequenceEqual(m.GetParameters())))];
